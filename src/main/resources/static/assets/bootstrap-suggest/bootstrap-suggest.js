    ...
    var methods = {
        init: function(options) {
            ...
            return self.each(function() {
                ...
                // 下拉按钮点击时
                if(options.clickDropdown !== false) {
                ...
                if ($iClear.length) {
                    ...
                    $parent.mouseenter(function() {
                        if (!$input.prop(DISABLED)) {
                        	if(options.clearRight) {
                        		$iClear.css('right', options.clearRight * 1).show();
                        	} else {
                        		$iClear.css('right', options.showBtn ? Math.max($input.next('.input-group-btn').width(), 33) + 10 : 12)
                                .show();
                        	}                           
                        }
                    }).mouseleave(function() {
