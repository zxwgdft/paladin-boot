...
public class DemoPersonnelController extends ControllerSupport {
	...
	public Object findPage(DemoPersonnelQuery query) {
		return CommonResponse.getSuccessResponse(demoPersonnelService.searchPage(query));
	}

	@GetMapping("/get")
	@ResponseBody
	public Object getDetail(@RequestParam String id, Model model) {
		return CommonResponse.getSuccessResponse(beanCopy(demoPersonnelService.get(id), new DemoPersonnelVO()));
	}
	...
	public Object save(@Valid DemoPersonnelDTO demoPersonnelDTO, BindingResult bindingResult) {
		...
		if (attachments != null && attachments.size() > 1) {
			return CommonResponse.getErrorResponse("附件数量不能超过1张");
		}
	...
	public Object delete(@RequestParam String id) {
		return CommonResponse.getResponse(demoPersonnelService.removeByPrimaryKey(id));
	}
			...
			if (query != null) {
				if (condition.isExportAll()) {
					return CommonResponse.getSuccessResponse("success",
							ExportUtil.export(condition, demoPersonnelService.searchAll(query), DemoPersonnel.class));
				} else if (condition.isExportPage()) {
					return CommonResponse.getSuccessResponse("success",
							ExportUtil.export(condition, demoPersonnelService.searchPage(query).getData(), DemoPersonnel.class));
				}
			}
	...
	static {
		...
		excelValueFormatMap.put(DemoPersonnel.COLUMN_FIELD_IDENTIFICATIONNO, new ValueFormator() {
			@Override
